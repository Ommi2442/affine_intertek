...
from utility.json_to_blob import save_local_jsons_and_docx_to_blob_and_cosmos_for_letter
...
def process_letter_direct(projectId: str, trf_urls, cdr_urls):
    ...
    try:
        ...
        if letter_percentage < 100:
            ...
            text_container = (
                f"vectorstorecontainer_new_itk_text_{first_name}_{projectId}"
            )
            image_container = (
                f"vectorstorecontainer_new_itk_image_{first_name}_{projectId}"
            )

            run_full_ingestion(
                projectId, Source_Doc_urls, text_container, image_container
            )

            blob_urls_trf = blob_urls + Source_Doc_urls

            f = main(projectId, blob_urls, text_container, image_container)
            print(f"Making progress beyond this point {f}")
            ...
            if f:
                ...
                letter_json2 = (
                    project_dir / f"letter_header_iec_output_{projectId}.json"
                )
                letter_docx_file = project_dir / f"letter_iec_output_{projectId}.docx"

                letter_json_path = (
                    BASE_DIR / "utility/letter_report/deploymentV1/letter.json"
                )
                letter_header_json_path = (
                    BASE_DIR / "utility/letter_report/deploymentV1/letter_header.json"
                )
