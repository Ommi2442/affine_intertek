...
def infer_category_from_path(path):
    ...
    if "client_docs" in path_lower:
        return "client_docs"

    elif "product_docs" in path_lower:
        return "product_docs"

    elif "component_docs" in path_lower:
        return "component_docs"

    elif "equipment_photos" in path_lower:
...
def upload_pdf_images_and_append_urls(
    image_page_metadata, image_urls, conn_str, container
):
    """
...
def upload_pdf_images_and_append_urls_parallel(
    image_page_metadata, image_urls, conn_str, container, max_workers=8
):
    """
...
def process_single_image(
    url, index, MAX_RETRIES, INITIAL_BACKOFF, total, vision_deploy_name, client
):
    """
...
def load_and_process_images(
    ...
    INITIAL_BACKOFF,
    client,
    vision_deploy_name=CHAT_DEPLOY,
):
    docs = []
...
def attach_supporting_refs_grey(
    ...
    return {
        ...
        "evidence_source": {
            "type": source_type,
            "files": {"image_files": list(image_files_needed)},
        },
...
def build_rag_image_pipeline_v5(
    retriever, llm, build_vision_message_v5, attach_supporting_refs, vs
):
    """
    FINAL v5:
        ...
        | RunnableMap(
            {
                "inputs": RunnableLambda(
                    lambda x: {**x, "context": extract_docs(x.get("context"))}
                ),
                "messages": RunnableLambda(lambda x: build_vision_message_v5(x)),
            }
        )
        # ----------------------------------------------------
...
def update_json_item_letter(item, result):
    ...
    for fname in json_image_files:
        ...
        if fname in pipeline_lookup:
            enriched.update(
                {k: v for k, v in pipeline_lookup[fname].items() if v is not None}
            )

        if enriched["url"] is None and fname in task_lookup:
            enriched.update(
                {k: v for k, v in task_lookup[fname].items() if v is not None}
            )
...
def extract_table1a(filename):
    ...
    data = [[cell.text.strip() for cell in row.cells] for row in target_table.rows]

    return pd.DataFrame(data)


def insert_dataframe_below_anchor_dep(
    input_docx: str, output_docx: str, df: pd.DataFrame, anchor_text: str
):
    """
...
def insert_dataframe_below_anchor(input_docx, output_docx, df, anchor_text):
...
def format_critical_components_df(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[
        ...
        & ~df["Name"].str.contains(
            r"IEC|Clause|table|NOTE|List all|May include|licence", case=False, na=False
        )
    ]
...
def fetch_actionable_requirements_dep(df):
    ...
    temp["Verdict"] = (
        temp["Verdict"].astype(str).str.replace(r"\s+", "", regex=True).str.upper()
    )
    ...
    for path in Path(directory).iterdir():
        ...
        try:
            ...
            if path.suffix.lower() == ".xls":
                ...
                tables = pd.read_excel(
                    path, sheet_name=None, header=None, engine="xlrd"
                )
        ...
        for sheet, df_raw in tables.items():
            ...
            for i, row in df_raw.iterrows():
                norm = row.astype(str).str.lower().str.replace(" ", "").str.strip()
...
def update_non_conforming_urls_from_blob(data, blob_urls):
    ...
    nonConformin_urls = [
        {"id": idx + 1, "url": unquote(url).rstrip("'")}
