...
def build_vision_message_grey(inputs, grey=False):
    ...
    task_type = (
        inputs.get("task_type") or inputs.get("prompt_type") or "extract"
    ).lower()
    ...
    evidence_block = EVIDENCE_BLOCK_TEMPLATE.format(
        text_files=text_files, image_files=image_files
    )
...
def attach_supporting_refs_grey(
    ...
    return {
        ...
        "evidence_source": {
            "type": source_type,
            "files": {"image_files": sorted(valid_image_files)},
        },
...
def build_rag_image_pipeline_grey(
    retriever, llm, build_vision_message, attach_supporting_refs, vs
):
        ...
        | RunnableMap(
            {
                ...
                "raw_llm_output": (itemgetter("messages") | llm | StrOutputParser()),
            }
        )
        # ----------------------------------------------------
...
def update_json_item_grey(item, result):
    ...
    # -----------------------------
    similarity_preview = (
        result.get("similarity_preview") or result.get("text_support") or []
    )
...
def build_tasks_with_custom_prompt_grey(data, image_urls):
    ...
    for img in image_urls:
        if isinstance(img, dict):
            ...
            clean = raw.split("?")[0]
            normalized_images.append(
                {"pdf_file": None, "page": None, "image_file": clean, "url": img}
            )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as exe:
            ...
            for idx, task in enumerate(batch):
                ...
                if stats:
                    ...
                    fut = exe.submit(
                        generator_evaluator_agent_tool, run_single_task_tool, task
                    )
...
def insert_marking_images_from_json_into_docx_new(
    docx_path, output_path, image_paths, table_index, row, col, width_inches=5
):
    """
...
def apply_checkboxes_from_json(docx_path, output_path, json_data):
    """
    ...
    for idx in checkbox_indices:
        set_checkbox_checked(
            docx_path=current_doc, out_path=output_path, checkbox_index=int(idx)
        )
...
def update_docx_tables_from_json_arial(docx_path, json_path, output_path):
    ...
    for table_obj in data["Tables"]:
        ...
        for item in table_obj["Items"]:
            ...
            if (
                not ai_fillable
                and task_type != "verdict_dependency"
                and not item.get("user_editable", False)
            ):
                continue
...
def insert_legacy_checkbox_with_text(
    docx_path, output_path, sentence, table_index=9, row=4, col=0, new_lines=4
):
    """
...
def fetch_marking_plate_via_vs2(retriever, output_filename="marking_plate.png"):
    """
...
def fetch_marking_plate_urls_and_update_json_new(
    ...
    for table in json_data.get("Tables", []):
        ...
        for item in table.get("Items", []):
            if (
                item.get("question_row") == question_row
                and item.get("question_column") == question_column
            ):
...
def update_pta_from_iec_correct(pta_path: str, iec_path: str, output_path: str):
    """
...
def update_pta_with_multiple_iec(
    ...
    for i, iec_path in enumerate(iec_paths, start=1):
        ...
        next_pta = (
            ...
            if is_last
            else project_data_dir / f"_tmp_pta_{i}.json"
        )
...
def delete_cosmos_container(
    ...
    try:
        ...
        print("\n===============================================")
        print(
            f"       STEP 12 — DELETING THE VECTOR COSMOS DB Container '{container_name}'"
        )
        print("===============================================")
...
def trf_gen_partwise(
    ...
    return {"json": output_json_path, "excel": excel_output_path, "stats": stats}
...
def run_trf_generation(
    ...
    input_json_path = Path(base_pta_path)
    excel_output_path = project_data_dir / f"{input_json_path.stem}.xlsx"

    print(
        f"[INFO] Single JSON mode: processing master PTA JSON directly: {input_json_path}"
    )
    ...
    stats = result["stats"]
    stats["file_numbers"] = len(blob_urls)

    print(f"[DONE] Single answered JSON generated: {final_output_path}")
    ...
    print(f"Total pipeline time: {total_time / 60:.2f} min")

    stats["time_taken"] = total_time
