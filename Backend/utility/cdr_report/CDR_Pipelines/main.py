...
from utility.cdr_report.CDR_Pipelines.utils import (
    build_ref,
    post_process_cdr,
    get_image_urls_from_container_sas,
    move_device_images_in_blob,
    get_blob_urls,
)
...
from utility.cdr_report.CDR_Pipelines.description import (
    description_main,
    build_product_section_items,
)
...
def main3(project_id, user_id, input_json, output_excel_path):
    ...
    step += 1
    progress(step, TOTAL, "Starting pipeline")
    ...
    count = utils.delete_folder_if_exists(
        AZURE_BLOB_CONNECTION_STRING, BLOB_CONTAINER_NAME, device_prefix
    )
    step += 1
    progress(step, TOTAL, "Device images cleaned", {"deleted": count})
    ...
    step += 1
    progress(step, TOTAL, "Listing blob URLs")
    ...
    step += 1
    progress(step, TOTAL, "Downloading TRF")
    ...
    step += 1
    progress(step, TOTAL, "Downloading images from blobs")
    ...
    step += 1
    progress(step, TOTAL, "Extracting CIS info")
    ...
    # --------------------------------------------------
    step += 1
    progress(step, TOTAL, "Saving extracted text")
    ...
    # --------------------------------------------------
    pdf_paths = [
        paths["SRC"] / f for f in os.listdir(paths["SRC"]) if f.lower().endswith(".pdf")
    ]
    ...
    # --------------------------------------------------
    step += 1
    progress(step, TOTAL, "Creating DB/container (Cosmos)")
    ...
    step += 1
    progress(step, TOTAL, "Building embeddings + vector store")
    # embeddings = utils.build_embeddings()
    vs = build_vectorstore()

    step += 1
    progress(step, TOTAL, "Chunking + ingesting documents into vector store")
    ...
    # --------------------------------------------------
    step += 1
    progress(step, TOTAL, "Building references")
    ref = build_ref(input_json)

    step += 1
    progress(step, TOTAL, "Generating references")
    ...
    # --------------------------------------------------
    step += 1
    progress(step, TOTAL, "Generating description")
    ...
    # --------------------------------------------------
    step += 1
    progress(step, TOTAL, "Running features")
    features = features_tools_main(vs, image_urls, run_audit=True)

    step += 1
    progress(step, TOTAL, "Moving device images")
    ...
    # --------------------------------------------------
    step += 1
    progress(step, TOTAL, "Running Sheets 3 & 4")
    ...
    # --------------------------------------------------
    step += 1
    progress(step, TOTAL, "Post processing")
    ...
    # --------------------------------------------------
    step += 1
    progress(step, TOTAL, "Generating Excel")
    ...
    utils.delete_cosmos_container(
        configs.COSMOS_URL, configs.COSMOS_KEY, configs.DB_NAME, Container_name
    )
